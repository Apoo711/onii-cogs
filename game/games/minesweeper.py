...
async def play(ctx, columns=None, rows=None, bombs=None):
    ...
    if columns < 1 or rows < 1 or bombs < 1:
        return await ctx.send(
            "The provided numbers cannot be zero or negative..."
        )
    ...
    embed.add_field(
        name="\U0001F4A3 Percentage:", value=f"{percentage}%", inline=True
    )
    embed.add_field(
        name="Requested by:", value=ctx.author.display_name, inline=True
    )
